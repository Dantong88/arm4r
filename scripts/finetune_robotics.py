...
from arm4r.data.datasets_robotic_finetuning import SequenceDataset

import arm4r.util.misc as misc
from arm4r.util.misc import NativeScalerWithGradNormCount as NativeScaler
from arm4r.util.args import ExperimentConfig
from arm4r.util.engine import train_one_epoch
...
def main(args: ExperimentConfig):
    ...
    assert len(
        rgb_observations) == args.shared_cfg.num_cameras, "Number of cameras must match the number of rgb observations"
    ...
    if args.model_cfg.policy_cfg.pretrained_path is not None:
        ...
        misc.load_model_pretrain_points(model_without_ddp, args.model_cfg.policy_cfg.pretrained_path, frozen=False)
    ...
    if global_rank == 0 and args.logging_cfg.log_name is not None:
        wandb.init(project="arm4r", config=args, name=args.logging_cfg.log_name,
    ...
    for epoch in range(args.shared_cfg.start_epoch, args.trainer_cfg.epochs):
        ...
        if resume_reload or epoch % args.shared_cfg.split_epoch == 0:
            ...
            sampler_train = misc.DistributedSubEpochSampler(
                dataset_train, num_replicas=num_tasks, rank=global_rank, split_epoch=args.shared_cfg.split_epoch,
                shuffle=True
            )
            sampler_val = misc.DistributedSubEpochSampler(
                dataset_val, num_replicas=num_tasks, rank=global_rank, split_epoch=args.shared_cfg.split_epoch,
                shuffle=False
            )
        ...
        if args.logging_cfg.output_dir and (
                epoch % args.shared_cfg.save_every == 0 or epoch + 1 == args.trainer_cfg.epochs):
            misc.save_model(
