...
def convert_delta_action_pretrain(action, proprio):
    """
    Calculate the delta action given the action and proprioception.

    action: n x t x m vector
    proprio: n x t x m vector
    """
    # Repeat the first proprioception state across the time dimension
    current_state = np.repeat(proprio[:, 0:1], action.shape[1], axis=1)  # Shape: n x t x m

    # Compute the delta values for the matching dimensions
    delta_action_core = action[..., :proprio.shape[-1]] - current_state

    # Handle the extra dimensions in action if present (e.g., EOS)
    if action.shape[-1] > proprio.shape[-1]:
        delta_action = np.concatenate(
            [delta_action_core, action[..., proprio.shape[-1]:]], axis=-1
        )
    else:
        delta_action = delta_action_core
...
def convert_abs_action_pretrain(delta_action, proprio):
    """
    Reconstruct the action from the delta action and proprioception.

    delta_action: n x t x m vector
    proprio: n x t x m vector
    """
    # Repeat the first proprioception state across the time dimension
    current_state = np.repeat(proprio[:, 0:1], delta_action.shape[1], axis=1)  # Shape: n x t x m

    # Reconstruct the core part of the action
    action_core = delta_action[..., :proprio.shape[-1]] + current_state

    # Handle the extra dimensions in delta_action if present (e.g., EOS)
    if delta_action.shape[-1] > proprio.shape[-1]:
        action = np.concatenate(
            [action_core, delta_action[..., proprio.shape[-1]:]], axis=-1
        )
    else:
        action = action_core
...
def convert_abs_action_pretrain_backup(action, proprio_0, proprios_1):
    '''
    Calculate the next state from the delta action and the current proprioception
    action: S, T, action_dim
    proprio: S, T, proprio_dim
...
def create_prompt_mask(eos_vector,num_steps):
    ...
    if len(eos_position) == 0:
        return np.ones_like(eos_vector), np.ones_like(eos_vector)
    else:
        # assert len(eos_position) == 1
        eos = eos_position[0]
        weight_mask = np.ones_like(eos_vector)
        weight_mask[eos + 1:] = 0
        return weight_mask, np.ones_like(eos_vector)
    # eos_position = sorted(np.where(eos_vector == 1)[0])
    # if len(eos_position) == 0:
