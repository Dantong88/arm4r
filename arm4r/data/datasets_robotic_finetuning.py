...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        common_path = glob(os.path.join(self.dataset_root, "**/*proprio.zarr"), recursive=True)
        self.common_path = [os.path.join(self.dataset_root, p.replace("/proprio.zarr", "")) for p in common_path]
        self.file2length = {}
        for file in self.common_path:
            self.file2length[file] = self.get_traj_length(file)
    ...
    def get_traj_length(
        ...
        action_fp = file_path + "/action.zarr"
