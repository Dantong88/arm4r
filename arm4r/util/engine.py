...
from arm4r.models.policy import ARM4R

def train_one_epoch(model: ARM4R,
    ...
    for data_iter_step, dataset_item in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if log_writer is not None and (data_iter_step + 1) % accum_iter == 0:
            ...
            if not validate:
                ...
                val_loss_sum += loss_value_reduce
                val_loss_count += 1
                for k, v in loss_value_dict.items():
                    if k not in val_metrics:
                        val_metrics[k] = 0.0
                    val_metrics[k] += v
            #     loss_val.append(loss_value_reduce)
    ...
    if log_writer is not None and validate:
        ...
        avg_loss = val_loss_sum / val_loss_count
        avg_metrics = {k: v / val_loss_count for k, v in val_metrics.items()}

        if log_writer is not None:
            epoch_1000x = int(epoch * 1000)
            log_writer.add_scalar('val_loss', avg_loss, epoch_1000x)
