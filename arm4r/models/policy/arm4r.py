...
class ARM4R(nn.Module):
    ...
    def __init__(
        ...
        self.arm4r_vision_norm = nn.LayerNorm(normalized_shape=self.vision_out_dim, eps=1e-6) # consistent with timm ViT Formulation
    ...
    def preprocessing(
        ...
        if self.separate_camera_adapter:
            if self.remove_proprio:
                ...
                f_obs = [self.arm4r_attn_pooling[i].combine_forward(f_obs[:, :, i], f_prop) for i in range(self.num_cameras)]
    ...
    def postprocessing(self, f_sa : torch.Tensor, B : int, T : int) -> tuple:
        ...
        out_a = self.arm4r_action_decoder(out_a).squeeze().view(B, T, self.action_dim * self.num_pred_steps).view(B, T, self.num_pred_steps, self.action_dim)
    ...
    def forward(self, sequences : dict) -> torch.Tensor:
        ...
        instruction = sequences['instruction'] # B, 512
    ...
    def forward_inference(self, sequences : dict, start_pos: int):
        ...
        if action_T == T - 1:
            # predicting action
            out_a_latent = out[:, -1, 1:-1, :].view(B, (self.latent_len - 1) * self.latent_dim) # B, latent_len, self.latent_dim
        ...
        #     out_proprio = self.arm4r_proprio_decoder.pred(out_s).view(B, self.num_pred_steps, self.proprio_dim)
    ...
    def get_action_eval(self, observation, abs_gripper_control=False, use_temporal=True, binary_gripper=False, teacher_forcing=False):
        ...
        if teacher_forcing:
            if observation["action"] is not None:
                observation["action"] = torch.cat([gt_action.unsqueeze(0), torch.zeros((1, 1, 1), device=observation["action"].device)], dim=-1)
        else:
            observation["action"] = self.last_action
        ...
        if binary_gripper:
            ...
            action[:,-1] = action[:,-1] > 0.4 # this might be changed based on your gripper threshold
