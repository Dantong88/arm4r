...
from arm4r.util.args import ExperimentConfig
import arm4r.util.misc as misc
from arm4r.util.model_constructor import model_constructor
from arm4r.data.utils import rot_6d_to_euler, quat_to_rot_6d, euler_to_rot_6d, convert_delta_action_pretrain
from arm4r.data.utils import convert_delta_action
...
class ARM4RWrapper(nn.Module):
    ...
    def prepare_observations(
        ...
        instruction = torch.tensor(instruction)[None, None].float() # B * T * 512
    ...
    def __call__(
        ...
        proprio : Union[np.ndarray], 
        action : Optional[np.ndarray] = None,
        instruction: Optional[np.ndarray] = None,
        ...
        Args:
            ...
            action (np.ndarray): action information
        """ 
        obs = self.prepare_observations(side_image, wrist_image, proprio, instruction, action)
...
class ARM4RWrapper_pretrain(nn.Module):
    def __init__(
            self,
            train_yaml_path: Union[str, Path],
            checkpoint_path: Union[str, Path],
        ...
        if self.preprocess is not None:
            ...
            self.preprocess_tensor = transforms.Compose(
                [t for t in self.preprocess.transforms if
                 not isinstance(t, transforms.ToTensor) and not isinstance(t, transforms.ColorJitter)]
            )
        else:
            print("warning: vision transforms are not defined. Using default transforms.")
            self.preprocess_PIL = transforms.Compose([
    ...
    def prepare_observations(
            ...
            side_image: Union[PIL.Image.Image, List[PIL.Image.Image], np.ndarray],
            wrist_image: Union[PIL.Image.Image, List[PIL.Image.Image], np.ndarray],
            proprio_0: Union[np.ndarray],
            proprio_1: Union[np.ndarray],
            instruction: Union[np.ndarray],
            action: Optional[np.ndarray] = None,
    ):
        """
        ...
        # find the length of the sequence
        seq_len = len(side_image)
        assert len(side_image) == len(wrist_image) == len(proprio_0) == len(proprio_1), "Length of the sequence must be the same"
        if action is not None:
            assert len(action) + 1 == seq_len or len(
                action) == seq_len, "Length of the action sequence must be one less than the observation sequence"
        assert proprio_0.shape[
                   0] == seq_len, "Length of proprio sequence must match the length of the observation sequence"

        # construct input dictionary
        side_image = torch.cat([preprocess(img)[None] for img in side_image], dim=0)
        wrist_image = torch.cat([preprocess(img)[None] for img in wrist_image], dim=0)
        # interleave the images
        image_vec = torch.stack([side_image, wrist_image], dim=1)[
            None].float()  # stack along a new axis (seq_len, 2, 3, H, W), then add batch dim

        # process proprio
        proprio_0 = np.reshape(proprio_0, [proprio_0.shape[0], -1])
        proprio_1 = np.reshape(proprio_1, [proprio_1.shape[0], -1])

        proprio_0 = torch.tensor(proprio_0)[None].float()
        proprio_1 = torch.tensor(proprio_1)[None].float()
        instruction = torch.tensor(instruction)[None, None].float()  # B * T * 512

        # process action
        if action is not None:
            action = np.reshape(action, [action.shape[0], -1])
            action_0 = action[:, :int(action.shape[1] / 2)]
            action_1 = action[:, int(action.shape[1] / 2):]
            if self.args.shared_cfg.use_delta_action:
                action_0 = convert_delta_action_pretrain(action_0[:, None, ...], proprio_0.numpy().transpose(1, 0, 2)).squeeze()[
                    None, ...]
                action_1 = \
                convert_delta_action_pretrain(action_1[:, None, ...], proprio_1.numpy().transpose(1, 0, 2)).squeeze()[
                    None, ...]
    ...
    def __call__(
        ...
        Args:
            ...
            action (np.ndarray): action information
        """
        obs = self.prepare_observations(side_image, wrist_image, proprio_0, proprio_1, instruction, action)
        for k, v in obs.items():
            if v is not None:
                obs[k] = v.to(self.device, non_blocking=True)
        action = self.model.get_action_eval(
...
class ARM4RWrapper_pretrain_single_view(nn.Module):
    def __init__(
            self,
            train_yaml_path: Union[str, Path],
            checkpoint_path: Union[str, Path],
    ...
    def prepare_observations(
        ...
        # process proprio
        proprio = np.reshape(proprio, [proprio.shape[0], -1])
        ...
        if action is not None:
            ...
            if self.args.shared_cfg.use_delta_action:
                action = \
                convert_delta_action_pretrain(action[:, None, ...], proprio.numpy().transpose(1, 0, 2)).squeeze()[
                    None, ...]
                action = torch.tensor(action).float()
            else:
                action = torch.tensor(action)[None].float()
