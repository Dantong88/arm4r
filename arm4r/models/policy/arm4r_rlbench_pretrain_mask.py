# test script:
# CUDA_VISIBLE_DEVICES=4 python -m llama.arm4r
...
from arm4r.models.backbones.encoders import VisionEncoder, VisionEncoderCNN, AttentionPool, MultiKVAttentionPool
from arm4r.models.losses import losses
from arm4r.data.utils import convert_abs_action, convert_delta_action, scale_action, unscale_action, load_json, \
    convert_abs_action_pretrain, convert_delta_action_pretrain


class ARM4R(nn.Module):
    """
    ...
    def __init__(
        ...
        self.scratch_llama_config = os.path.join(scratch_llama_config)
        ...
        if camera_pos_emb:
            self.arm4r_camera_pos_emb = nn.Parameter(torch.randn(num_cameras, 1, self.vision_out_dim))
            nn.init.trunc_normal_(self.arm4r_camera_pos_emb, std=0.2)

        # proprioception encoder
        self.proprio_dim = proprio_dim  # (including x,y,z)

        self.remove_proprio = remove_proprio

        self.arm4r_text_encoder = Mlp(in_features=512, out_features=self.vision_out_dim)

        if not self.remove_proprio:
            self.arm4r_proprio_encoder = Mlp(in_features=self.proprio_dim, out_features=self.vision_out_dim)
        ...
        if self.separate_camera_adapter:
            self.arm4r_attn_pooling = nn.ModuleList(
                [attn_pool_cls(
            ...
            self.arm4r_attn_pooling = attn_pool_cls(
                self.vision_out_dim, out_features=self.latent_dim, mlp_ratio=adapter_mlp_ratio,
                latent_len=self.attn_latent_len, num_heads=adapter_num_heads,
            )
        self.arm4r_vision_norm = nn.LayerNorm(normalized_shape=self.vision_out_dim,
                                             eps=1e-6)  # consistent with timm ViT Formulation
        ...
        self.arm4r_action_encoder = Mlp(in_features=self.action_dim, out_features=self.latent_dim)
        ...
        if self.modality_pos_emb:
            ...
            self.arm4r_pos_emb_state = nn.Parameter(torch.randn(self.latent_dim))
            self.arm4r_pos_emb_act = nn.Parameter(torch.randn(self.latent_dim))

            # initialize them
            nn.init.trunc_normal_(self.arm4r_pos_emb_state, std=0.2)
            nn.init.trunc_normal_(self.arm4r_pos_emb_act, std=0.2)
    ...
    def _construct_decoder_heads(self):
        ...
        if not self.pred_action_only:
            self.arm4r_proprio_decoder = _action_head_constructor(
        ...
        self.arm4r_action_decoder = _action_head_constructor(
            head_type=self.decoder_pred_head,
            input_dim=self.latent_dim,
            hidden_features=self.decoder_hidden_features,
    ...
    def state_dict(self, destination=None, prefix='', keep_vars=False):
        state_dict = super(ARM4R, self).state_dict(destination, prefix, keep_vars)
        trainable_params = self.get_trainable_params(
            self.phase)  # trainable_params points to different tensor as state_dict
    ...
    def preprocessing(
        ...
        """
        This function preprocesses the observation, proprioception, and action data for the arm4r model.
        ...
        # proprio processing
        if not self.remove_proprio:
            f_prop = self.arm4r_proprio_encoder(proprio)  # B, T, self.vision_out_dim

        # observation processing
        f_obs = self.vision_encoder(observation)  # B, T, N, K, self.vision_out_dim (k is number of patches + cls token)
        f_obs = self.arm4r_vision_norm(f_obs)
        if self.camera_pos_emb:
            f_obs = f_obs + self.arm4r_camera_pos_emb  # add camera positional embeddings

        if self.separate_camera_adapter:
            if self.remove_proprio:
                f_obs = [self.arm4r_attn_pooling[i].forward_visual(f_obs[:, :, i]) for i in range(self.num_cameras)]
            else:
                f_obs = [self.arm4r_attn_pooling[i].combine_forward(f_obs[:, :, i], f_prop) for i in
            ...
            # state extraction
            if self.remove_proprio:
                f_s = self.arm4r_attn_pooling.forward_visual(f_obs)  # B, T, latent_len, self.latent_dim
            else:
                f_s = self.arm4r_attn_pooling.combine_forward(f_obs, f_prop)  # B, T, latent_len, self.latent_dim

        # action processing
        f_a = self.arm4r_action_encoder(action)  # B, T, D
        f_a = f_a[:, :, None, :]  # B, T, 1, self.latent_dim

        # instruction processing
        f_instruction = self.arm4r_text_encoder(instruction)  # B, T, 768
        f_instruction = f_instruction[:, :, None, :]  # B, T, 1, self.latent_dim

        # interweave state and action on the temporal dimension
        if self.modality_pos_emb:
            f_s = f_s + self.arm4r_pos_emb_state
            f_a = f_a + self.arm4r_pos_emb_act

        f_sa = torch.cat([f_instruction, f_s, f_a], dim=2)  # B, T, latent_len+1, self.latent_dim
    ...
    def postprocessing(self, f_sa: torch.Tensor, B: int, T: int) -> tuple:
        ...
        out_a = self.arm4r_action_decoder(out_a).squeeze().view(B, T, self.action_dim * self.num_pred_steps).view(B, T,
                                                                                                                 self.num_pred_steps,
                                                                                                                 self.action_dim)

        # decoding to proprio
        if not self.pred_action_only:
            out_proprio = self.arm4r_proprio_decoder(out_s).view(B, T - 1, self.num_pred_steps, self.proprio_dim)
            out_eos = out_a[:, :, :, -1]
            out_action = out_a[:, :, :, :-1]
        else:
            out_proprio = None
    ...
    def preprocessing_attention(self,
        ...
        f_obs = f_obs.view(f_obs.shape[0], f_obs.shape[1], -1, f_obs.shape[-1])  # B, T, N*K, self.vision_out_dim
        f_s = self.arm4r_attn_pooling.forward_attention(f_obs, f_prop)  # B, T, latent_len, self.latent_dim
    ...
    def forward(self, sequences: dict) -> torch.Tensor:
        ...
        weight_mask = sequences['weight_mask']  # B, T
        instruction = sequences['instruction']  # B, T, 512
        points_mask = sequences['points_mask'] # B, 1296
        points_mask = points_mask.unsqueeze(2)
        ...
        # preprocessing observations, proprio and actions
        f_sa = self.preprocessing(observation, proprio_in, action_in, instruction)
        ...
        out_s = out[:, :, 0, :].reshape(B * T, self.latent_dim)
        out_i = out[:, :-1, 2, :].reshape(B * (T - 1), self.latent_dim)
        ...
        if not self.pred_action_only:
            ...
            state_loss = self.arm4r_proprio_decoder.loss(out_s, proprio_target)
        else:
            state_loss = 0  # torch.tensor(0.0, dtype=proprio.dtype, device=proprio.device)

        if self.scale_action is not None:
            action_target = scale_action(action, self.scale_action)

        action_target = action.reshape(B * T, self.action_dim * self.num_pred_steps)
        action_loss = self.arm4r_action_decoder.loss(out_a, action_target)

        if self.no_prompt_loss:
            state_loss = state_loss * prompt_mask[:, :-1].reshape(B * (T - 1), 1)
            action_loss = action_loss * prompt_mask.view(B * T, 1) * points_mask.repeat(1, 1, N, 3).view(B * T, -1)
        ...
        if self.no_prompt_loss:
            ...
            action_loss = action_loss.sum() / (prompt_mask.sum() * (points_mask[0, 0, 0].sum() * 3) * self.num_pred_steps + 1e-6)
        else:
    ...
    def forward_inference(self, sequences: dict, start_pos: int):
        ...
        instruction = sequences.get("instruction")
        ...
        # preprocessing observations, proprio and actions
        f_sa = self.preprocessing(observation, proprio, action, instruction)
        ...
        if action_T == T - 1:
            # predicting action
            out_a_latent = out[:, -1, 1:-1, :].view(B, (
                    self.latent_len - 1) * self.latent_dim)  # B, latent_len, self.latent_dim
            out_a = self.arm4r_action_decoder.pred(out_a_latent).view(B, self.num_pred_steps, self.action_dim)
    ...
    def get_action(self, observation: dict) -> torch.Tensor:
        ...
        self.last_action = torch.cat(
            [action.clone().unsqueeze(0).unsqueeze(0), torch.zeros((1, 1, 1), device=action.device)], dim=-1)  # add eos
    ...
    @torch.inference_mode()
    def get_action_eval(self, observation, abs_gripper_control=False, use_temporal=True, binary_gripper=False,
        ...
        if teacher_forcing:
            if observation["action"] is not None:
                observation["action"] = torch.cat(
                    [gt_action.unsqueeze(0), torch.zeros((1, 1, 1), device=observation["action"].device)], dim=-1)
        else:
            observation["action"] = self.last_action

        with torch.amp.autocast('cuda', dtype=torch.bfloat16):
            action_sequence, eos_sequence = self.forward_inference(observation, self.start_pos)
        # here need to be change when pred mutiple steps
        # action = action_sequence[0].squeeze() # batch_size *10
        ...
        if self.use_delta_action:
            ...
            action = convert_abs_action_pretrain(delta_action[None].cpu().numpy(), observation['proprio'].cpu().numpy())
        ...
        if not use_temporal:
            ...
            print(action[-1].cpu().numpy())

        # convert to delta action
        if self.use_delta_action:
            last_action = convert_delta_action_pretrain(action[None, None].cpu().numpy(), observation['proprio'].cpu().numpy())
        ...
        self.last_action = torch.cat(
            [last_action.clone().unsqueeze(0).unsqueeze(0), torch.zeros((1, 1, 1), device=last_action.device)], dim=-1)
